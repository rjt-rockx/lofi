...
export const Cover: FunctionComponent<Props> = ({ settings, onVisualizationChange, onVisualizationCycle }) => {
  ...
  const [volumeChangedTimeout, setVolumeChangedTimeout] = useState<NodeJS.Timeout>(null);
  ...
  // TODO touchpad on mac sucks, throttle it
  useEffect(() => {
    if (!volumeChangedTimeout) {
      const timeout = setTimeout(() => {
        setIsVolumeChanged(false);
        setVolumeChangedTimeout(null);
      }, 3000);
      setVolumeChangedTimeout(timeout);
    }
  }, [volumeChangedTimeout]);
        ...
        <>
          {shouldShowTrackInfo && (
            <WindowPortal name="track-info">
              <TrackInfo track={state.track} artist={state.artist} isOnLeft={isOnLeft} />
            </WindowPortal>
          )}
          <CoverContent
            className="full"
          ...
          />
          <Controls skipSongDelay={skipSongDelay} onTrackLiked={refreshTrackLiked} onPlaybackChanged={listeningTo} />
        </>
      ) : (
        <Waiting />
      )}
